...
/**
 * Send an SMS from a JustCall number.
 * ⚠️ UNVERIFIED until scripts/justcall-sms-probe.js confirms the endpoint/shape
 * against the live API — run the probe (one text to YOUR OWN phone) first.
 * Sender comes from JUSTCALL_SMS_FROM in .env.
 */
function sendText(to, body) {
  const from = (process.env.JUSTCALL_SMS_FROM || '').trim();
  if (!from) throw new Error('JUSTCALL_SMS_FROM not set in .env (run scripts/justcall-sms-probe.js --numbers)');
  return jc('POST', '/v2.1/texts/new', {
    body: { justcall_number: from, contact_number: to, body },
  });
}

module.exports = { jc, listCalls, getCall, getCallAi, transcriptToText, sendText };
